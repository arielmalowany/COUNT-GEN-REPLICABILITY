...
class Custom(data.Dataset):
    ...
    def __getitem__(self, index):
        img_name = str(self.images[index])
        img = self.tf(Image.open(os.path.join(self.data_path, img_name)))
        att = torch.tensor((self.labels[index] + 1) // 2)
        return img, att, img_name
...
class CelebA_HQ_custom(data.Dataset):
    def __init__(self, data_path, attr_path, image_size, mode, selected_attrs):
        super(CelebA_HQ_custom, self).__init__()
        self.data_path = data_path
        att_list = open(attr_path, 'r', encoding='utf-8').readlines()[1].split()
        atts = [att_list.index(att) + 1 for att in selected_attrs]
        orig_atts = np.loadtxt(attr_path, skiprows=2, usecols=atts, dtype=int)
        
        images = ['{:d}.jpg'.format(i) for i in range(30000)]
        
        if mode == 'train':
            self.images = images[:28000]
            self.img_atts = orig_atts[:28000]
        if mode == 'valid':
            self.images = images[28000:28500]
            self.img_atts = orig_atts[28000:28500]
        if mode == 'test':
            self.images = images[28500:]
            self.img_atts = orig_atts[28500:]
    ...
    def __getitem__(self, index):
        ...
        img = self.tf(Image.open(os.path.join(self.data_path, img_name)))
        atts = self.img_atts[index]
        return img, atts, [img_name]
    def __len__(self):
        return self.length
